...
- 절대 단편 키워드로 추측하지 말 것.
- 경쟁사 데이터가 있으면 메시지 차이, 키 카피, CTA 전략의 차이를 구체적으로 비교하라.
...
[경쟁사 ${c.rank}: ${c.domain}]
- 네이버 검색결과 제목: ${c.title}
- 네이버 검색결과 설명(파워링크): ${c.description}
...
⚠️ 작성 원칙:
1. 모든 진단은 위 데이터에 실제로 있는 값을 인용. 추측 금지.
2. 점수는 차등 평가 (전부 70점대 X).
3. checklist 12개 항목 모두 작성. currentValue는 위 데이터에서 실제 값을 그대로 인용.
4. criticalIssues 3~5개. 각 이슈에 badExample(현재 사이트 실제 잘못된 예)과 goodExample(개선된 예시)을 반드시 포함.
5. quickWinsDetailed는 4~6개. 각 항목은 단계(steps) 3~4개로 분해.
6. priorityRoadmap은 짧고 간결하게 (시점 기준 분류만, 액션 2~5개씩).
7. exampleCopy에는 currentHeroHeadline(현재 h1 또는 title)을 반드시 넣고, 경쟁사 데이터 있다면 competitorCopyInsight도 작성.

{
  ...
  "checklist": [
    { "id": "title", "category": "seo", "label": "title 태그",
      "status": "pass|warning|fail", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "meta_description", "category": "seo", "label": "Meta Description",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "og_tags", "category": "seo", "label": "Open Graph 태그",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "h1", "category": "content", "label": "H1 헤드라인",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "image_alt", "category": "content", "label": "이미지 ALT 텍스트",
      "status": "...", "currentValue": "<N개 중 M개 누락>", "diagnosis": "...", "guide": "..." },
    { "id": "viewport", "category": "seo", "label": "모바일 viewport",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "cta_clarity", "category": "conversion", "label": "CTA 명확도",
      "status": "...", "currentValue": "<감지된 CTA>", "diagnosis": "...", "guide": "..." },
    { "id": "cta_repeat", "category": "conversion", "label": "CTA 반복 노출",
      "status": "...", "currentValue": "<CTA 개수>", "diagnosis": "...", "guide": "..." },
    { "id": "contact_info", "category": "conversion", "label": "연락처 정보",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "trust_review", "category": "trust", "label": "후기/리뷰 노출",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "trust_certification", "category": "trust", "label": "인증/수상/파트너",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." },
    { "id": "price_info", "category": "conversion", "label": "가격/견적 정보",
      "status": "...", "currentValue": "...", "diagnosis": "...", "guide": "..." }
  ],
  "criticalIssues": [
    {
      ...
      "badExample": "<현재 사이트의 실제 잘못된 예 - 큰따옴표로. 예: '문의하기'>",
      "goodExample": "<개선된 예시 - 큰따옴표로. 예: '30초 무료 진단받기 →'>",
      "exampleNote": "<왜 이렇게 바꾸는 게 좋은지 한 줄>"
    }
  ],
  "quickWinsDetailed": [
    {
      "title": "<오늘 바로 적용 가능한 개선>",
  ...
  "exampleCopy": {
    "currentHeroHeadline": "<사이트의 실제 h1 또는 title>",
    "currentCtaText": "<감지된 CTA 중 대표 1개 또는 '(CTA 없음)'>",
    "heroHeadline": "<개선된 메인 헤드라인>",
    "subHeadline": "<서브 카피>",
    "ctaText": "<개선된 CTA 6~12자>",
    "competitorCopyInsight": "<경쟁사들이 강조하는 메시지 vs 우리의 차이 2-3문장. 경쟁사 없으면 빈 문자열>"
  },
  "finalCta": {
  ...
  "competitorAnalysis": {
    ...
    "competitors": [
      ${competitorAnalysis.competitors
        ...
        "keyMessage": "<이 경쟁사가 강조하는 메시지 한 줄 - h1/h2/메타 종합 판단>",
        "differentiation": "<우리 사이트와 어떻게 다른지 구체적으로 한 줄>"
      }`
        )
        .join(",\n      ")}
    ],
    "overallComparison": "<경쟁사 공통 강조점과 우리가 놓치는 부분 3-4문장>",
    "ourPositioning": "<우리가 취해야 할 차별화 포지셔닝 제안 3-4문장>"
  }`
      : ""
  }
}
